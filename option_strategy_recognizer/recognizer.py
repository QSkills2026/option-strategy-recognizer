"""Option strategy recognition — groups raw positions into named strategies."""
...
# Cash-like ETFs have no directional equity exposure — net_delta forced to 0
CASH_LIKE_TICKERS = {
...
class OptionStrategyRecognizer:
    """Recognize named options strategies from a flat list of positions.

    Groups positions by underlying, then classifies each group as a named
    strategy (Iron Condor, Bull Put Spread, Covered Call, etc.) with net
    Greeks, max profit/loss, breakevens, and DTE computed automatically.
    ...
    Usage::
        ...
        for g in groups:
            print(g.underlying, g.strategy_type, g.net_credit)
    """
    ...
    def _consolidate_positions(self, positions: List[PositionRecord]) -> List[PositionRecord]:
        ...
        Two-step process:
        1. Deduplicate exact rows (same contract key + same position size).
           Some brokers (e.g. IB Flex XML) return each lot twice.
    ...
    def _attach_modifiers(self, strategies: List[StrategyGroup],
        ...
        for mod_sg in single_longs:
            ...
            if target:
                ...
                used.add(id(mod_sg))
        return non_single + [sg for sg in single_longs if id(sg) not in used]
    ...
    def _compute_metrics(self, sg: StrategyGroup) -> None:
        ...
        sg.net_credit = sum(-p.position * p.cost_basis_price * p.multiplier for p in opt_legs)
